...
def Computer_play():
        Computer_Card = random.choices(number)
        # FIX: use global Computer_Cards safely
        Comp_Card = Computer_Card + Computer_Cards
        Comp_score = sum(Comp_Card)
        return Comp_Card , Comp_score

def Card_2():
    global Game, Phase, Your_Cards, Computer_Cards, Computer_score
    ...
    # FIX: update Your_Cards so next hit works
    Your_Cards = Card
...
while not Game:
    start  = input("Do you want to play a Game of blackjack? type y for yes or n for no \n")
    ...
    # FIX: Computer_Cards must be a LIST because you use indexing later
